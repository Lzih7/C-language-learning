 ...
 *
 * _Pin     : GPIO 引脚号，用于指定需要配置的引脚（如 GPIO_Pin_0, GPIO_Pin_1 等）。
 * _Mode    : GPIO 引脚工作模式（如输入、输出、复用等），通过宏参数传入。
 ...
 *
 * _TIMX            : 目标TIMx,比如 TIM1 TIM2 ...
 * _OCX             : 定时器通道, 1 2 3 4
 * _OCMode          : 输出比较模式，用于选择定时器的 PWM 模式，例如 TIM_OCMode_PWM1 或 TIM_OCMode_PWM2。
 * _OutputState     : 指定输出状态（使能或禁用），用来决定 PWM 信号是否输出。例如 TIM_OutputState_Enable 可以启用 PWM 输出。
 * _Pulse           : 设置 PWM 占空比，表示高电平持续的时间（脉宽），值越大占空比越高。
 * _OCPolarity      : 设置 PWM 输出的极性，表示输出信号是高电平有效还是低电平有效（如 TIM_OCPolarity_High）。
 *
 *
 * TIM_OC##_OCX##Init():根据指定的通道（_OCX）调用相应的初始化函数，例如 TIM_OC1Init, TIM_OC2Init 等。
 *                      _OCX 是宏参数，可以拼接为 TIM_OC1Init(), TIM_OC2Init, TIM_OC3Init
 *
 */
 ...
 *
 *
 * NVIC_InitTypeDef     : 这是 STM32 标准外设库中的结构体，用于配置 NVIC（嵌套向量中断控制器）的参数，
 *                        包括中断通道、优先级和启用状态。
...
#define UTIL_USART_CFG(_BaudRate, _WordLength, _USART_StopBits, _Parity, _FlowControl, Mode) \
    do                                                                                       \
    {                                                                                        \
        ...
        USART_InitStructure.USART_HardwareFlowControl = _FlowControl;                        \
        USART_InitStructure.USART_Mode = Mode;                                               \
        USART_Init(USART1, &USART_InitStructure);                                            \
    } while (0)

#define UTIL_DMA_CFG(_DMAx_Channelx,                                                          \
                     _PeripheralBaseAddr, _MemoryBaseAddr, _DIR, _BufferSize, _PeripheralInc, \
                     _MemoryInc, _PeripheralDataSize, _MemoryDataSize, _Mode, _Priorit, _M2M) \
    do                                                                                        \
    {                                                                                         \
        DMA_InitTypeDef DMA_InitStructure;                                                    \
        DMA_InitStructure.DMA_PeripheralBaseAddr = _PeripheralBaseAddr;                       \
        DMA_InitStructure.DMA_MemoryBaseAddr = _MemoryBaseAddr;                               \
        DMA_InitStructure.DMA_DIR = _DIR;                                                     \
        DMA_InitStructure.DMA_BufferSize = _BufferSize;                                       \
        DMA_InitStructure.DMA_PeripheralInc = _PeripheralInc;                                 \
        DMA_InitStructure.DMA_MemoryInc = _MemoryInc;                                         \
        DMA_InitStructure.DMA_PeripheralDataSize = _PeripheralDataSize;                       \
        DMA_InitStructure.DMA_MemoryDataSize = _MemoryDataSize;                               \
        DMA_InitStructure.DMA_Mode = _Mode;                                                   \
        DMA_InitStructure.DMA_Priority = _Priorit;                                            \
        DMA_InitStructure.DMA_M2M = _M2M;                                                     \
        DMA_Init(_DMAx_Channelx, &DMA_InitStructure);                                         \
    } while (0)

#define UTIL_ADC_CFG(_ADCx, _ADC_Mode, _ADC_ScanConvMode, _ADC_ContinuousConvMode, \
                     _ADC_ExternalTrigConv, _ADC_DataAlign, _ADC_NbrOfChannel)     \
    do                                                                             \
    {                                                                              \
        ADC_InitTypeDef ADC_InitStructure;                                         \
        ADC_InitStructure.ADC_Mode = _ADC_Mode;                                    \
        ADC_InitStructure.ADC_ScanConvMode = _ADC_ScanConvMode;                    \
        ADC_InitStructure.ADC_ContinuousConvMode = _ADC_ContinuousConvMode;        \
        ADC_InitStructure.ADC_ExternalTrigConv = _ADC_ExternalTrigConv;            \
        ADC_InitStructure.ADC_DataAlign = _ADC_DataAlign;                          \
        ADC_InitStructure.ADC_NbrOfChannel = _ADC_NbrOfChannel;                    \
